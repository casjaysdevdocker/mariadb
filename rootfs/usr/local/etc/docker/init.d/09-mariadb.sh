...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202410091112-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  WTFPL
# @@ReadME           :  09-mariadb.sh --help
# @@Copyright        :  Copyright: (c) 2024 Jason Hempstead, Casjays Developments
# @@Created          :  Wednesday, Oct 09, 2024 11:12 EDT
# @@File             :  09-mariadb.sh
# @@Description      :
...
EXEC_CMD_BIN='mariadb'                                       # command to execute
...
root_user_pass="${MARIADB_ROOT_PASS_WORD:-}" # root user password
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
[ -f "/config/env/mariadb.script.sh" ] && . "/config/env/mariadb.script.sh" # Generated by my dockermgr script
[ -f "/config/env/mariadb.sh" ] && . "/config/env/mariadb.sh"               # Overwrite the variabes
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
MYSQL_SCRIPT
			...
			mariadb -v -u $db_root_user <<MYSQL_SCRIPT
GRANT ALL PRIVILEGES ON $DATABASE_CREATE.* TO 'root'@'%';
MYSQL_SCRIPT
		fi
		mariadb -v -u $db_root_user <<MYSQL_SCRIPT
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# root/admin user info [password/random]
#ENV_ROOT_USER_NAME="${ENV_ROOT_USER_NAME:-$MARIADB_ROOT_USER_NAME}"   # root user name
#ENV_ROOT_USER_PASS="${ENV_ROOT_USER_NAME:-$MARIADB_ROOT_PASS_WORD}"   # root user password
#root_user_name="${ENV_ROOT_USER_NAME:-$root_user_name}"                              #
#root_user_pass="${ENV_ROOT_USER_PASS:-$root_user_pass}"                              #
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#Normal user info [password/random]
#ENV_USER_NAME="${ENV_USER_NAME:-$MARIADB_USER_NAME}"                  #
#ENV_USER_PASS="${ENV_USER_PASS:-$MARIADB_USER_PASS_WORD}"             #
